...
- `config/` - YAML configuration files (system prompts, context templates)
...
### Tool System
**Definitions** (`backend/sdk/tools/`):
- `definitions.py` - Base `ToolDefinition` dataclass
- `action.py` - Common tools (skip, memorize, recall)
- `gameplay.py` - Action Manager tools (narration, suggest_options, travel, etc.)
- `onboarding.py` - Onboarding phase tools
- `subagent.py` - Sub-agent persist tools
- `guideline.py` - Guideline access tools

**Handlers** (`backend/sdk/handlers/`):
...
2. **Tool changes require both definition and handler** - define in `tools/`, implement in `handlers/`
