...
- Sub-agent invocation via `SubAgentInvoker`
...
Eight specialized agents collaborate in two phases: **Onboarding** (interview → world generation) and **Gameplay** (2-agent tape with sub-agent tools).
...
- Sub-agent invocation tools: `stat_calc`, `add_character`, `remove_character`, `add_location`
- Narrator tools: `suggest_actions`
- Action Manager coordination tools
