...
세계를 만드는데 있어서 엥간해서는 제한이 없음. 가상의 세계이고 실제 세계랑은 관련이 없다고 못박았기 때문에 만드는건 괜찮은데, 괜히 미성년자나 최면어플 이런거 넣으면 진행할때 계속 헷갈려하고 쓸데없이 오래 걸리기 때문에 가급적이면 안 넣는걸 추천


### 3. 월드 생성


클로드월드에서 월드는 '설정집, 스탯시스템, 장소, 캐릭터, 아이템 사전' 을 의미함

처음 온보딩이 끝나면 기본 설정집이 만들어지고 스탯 시스템이 만들어져서, 기본적으로 모든 진행에 있어서 이걸 전제로 하고 굴러감. 즉 시스템 프롬프트에 박힌다는 뜻

예를 들어 rpg 계열이면 스탯시스템에 체력, 마력 등이 만들어질거고, 매 진행시마다 체력이나 마력 데이터를 참조하면서 갱신할거임. 만약 불만이 있다면 온보딩이 끝난 후 직접 생성된 파일을 보고 수정할수 있음.


### 4. 게임 시작!


온보딩이 끝나면 'enter world' 라는 버튼이 생김. 이걸 누르면 게임이 시작됨

기본적으로 온보딩 및 첫 게임 시작에서 세계관을 모두 만들기 때문에 첫 게임 진행 시작까지 10분정도가 걸린다 보면 됨. 오푸스를 기본으로 쓰기 때문에 매 진행도 시간이 좀 걸리긴 하는데... 그래도 최대한 최적화를 했고 매 턴 처리가 평균 20초정도 걸리니 양해바람
...
나레이션과 함께 선택지가 주어짐. 선택지를 눌러도 되고 화면 하단의 입력창에 **하고 싶은 행동**을 자유롭게 입력해도 됨. ooc (out of character) 문구를 입력해도 되긴 하는데 엥간해서는 안 하는걸 추천
...
행동을 수행하는것 이외에도 현재 장소에 있는 캐릭터와 대화하는 기능이 있음. 이건 나레이션 개입 없이 캐릭터랑 1:1로 대화할 수 있는 기능이기 때문에, 미연시 계열로 만들었다면 이 기능을 자주 사용하길 추천.

1:1 대화가 끝나면 대화내용이 요약돼서 나레이터한테 다시 넘어가고 다시 나레이션과 다음 행동 추천이 제시됨. 이 과정에서 시간이 다소 걸리니 양해 부탁
...
- **에이전트** - 이거 중요함. 현재 장소에 내가 원하는 캐릭터가 없다면 1:1 채팅이 고장남. 게임 진행 자체는 문제가 없는데 1:1 채팅을 자주 쓰고 싶다면 사용하기 전 이 부분을 항상 체크할것
...
기본적으로 게임은 '장소 단위로 진행'됨. 무슨 말이냐하면, 유저가 장소를 옮길 때마다 이전 장소에 있었던 일이 요약되고 컨텍스트가 초기화됨.

장소를 옮기는데 있어서 별다른 행동이 필요한건 아님. 사소하게는 '짜증나니까 밖으로 나간다' 는 명령어만 넣어도 알아서 장소가 바뀜. 대신 기존에 만들어지지 않은 장소라면 나레이터가 장소를 만드느라 시간이 걸리니까 이 점 감안하길 바람
...
사실 전투가 따로 구현된게 아니고, 나레이터가 유저 행동에 따라 알아서 스탯 갱신을 하고 아이템 처리도 함. 캐릭터를 제거하기도 하고 알아서 생성하기도 할거임

때에 따라서 운에 맞길 때가 있는데 이 때는 나레이터가 알아서 주사위를 굴려서 결과에 약간의 킥을 줄거임
