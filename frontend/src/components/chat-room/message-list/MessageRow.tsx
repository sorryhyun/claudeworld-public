...
interface ParsedThinking {
  npcReactions: Array<{ agentName: string; content: string }>;
  regularThinking: string | null;
}

function parseThinking(thinking: string | null | undefined): ParsedThinking {
  if (!thinking) {
    return { npcReactions: [], regularThinking: null };
  }

  const result: ParsedThinking = { npcReactions: [], regularThinking: null };

  // Extract NPC reactions block
  const reactionsMatch = thinking.match(
    /\[NPC_REACTIONS\]([\s\S]*?)\[\/NPC_REACTIONS\]/,
  );

  if (reactionsMatch) {
    const reactionsText = reactionsMatch[1];
    // Parse individual reactions: === AgentName ===\ncontent
    const reactionPattern = /=== ([^=]+) ===\n([\s\S]*?)(?=\n=== |$)/g;
    let match;
    while ((match = reactionPattern.exec(reactionsText)) !== null) {
      result.npcReactions.push({
        agentName: match[1].trim(),
        content: match[2].trim(),
      });
    }

    // Get regular thinking (anything outside the NPC_REACTIONS block)
    const outsideReactions = thinking
      .replace(/\[NPC_REACTIONS\][\s\S]*?\[\/NPC_REACTIONS\]/, "")
      .trim();
    if (outsideReactions) {
      result.regularThinking = outsideReactions;
    }
  } else {
    // No NPC reactions markers - treat entire thinking as regular
    result.regularThinking = thinking;
  }
...
export const MessageRow = memo(
  ({
    ...
    // Parse thinking field to extract NPC reactions
    const parsedThinking = useMemo(
      () => parseThinking(message.thinking),
      [message.thinking],
    );

    const hasNpcReactions = parsedThinking.npcReactions.length > 0;
    const hasRegularThinking = !!parsedThinking.regularThinking;
    ...
    return (
      <div style={style} className="message-padding-mobile" data-index={index}>
        {message.participant_type === "system" ? (
          ...
          >
            <div
              className={`flex gap-2 sm:gap-3 w-full max-w-[92%] sm:max-w-[85%] lg:max-w-3xl ${message.role === "user" ? "flex-row-reverse" : ""}`}
            >
              {/* Avatar */}
              {message.role === "user" ? (
                ...
                <img
                  src={
                    getAgentProfilePicUrl({
                      name: message.agent_name,
                      profile_pic: message.agent_profile_pic,
                    }) || ""
                  }
              ...
              <div className="flex flex-col gap-1 min-w-0">
                ...
                {message.role === "user" && (
                  <div className="flex items-center gap-2 px-1 justify-end">
                    <span className="font-semibold text-sm text-slate-700">
                      {message.participant_type === "character" &&
                  ...
                  {message.role === "assistant" &&
                    ...
                    !message.is_chatting && (
                      <>
                        <button
                          onClick={() =>
                            onToggleThinking(`reactions-${message.id}`)
                          }
                          className="flex items-center gap-2 text-xs font-medium text-indigo-500 hover:text-indigo-700 transition-colors ml-1 mb-1"
                        >
                          <svg
                            className={`w-4 h-4 transition-transform ${expandedThinking.has(`reactions-${message.id}`) ? "rotate-90" : ""}`}
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                          ...
                          <span>
                            NPC Reactions ({parsedThinking.npcReactions.length})
                          </span>
                        </button>

                        {expandedThinking.has(`reactions-${message.id}`) && (
                          <div className="pl-3 py-2 my-1 border-l-2 border-indigo-300 bg-indigo-50/50 rounded-r-lg space-y-3">
                            {parsedThinking.npcReactions.map((reaction, idx) => (
                    ...
                    expandedThinking.has(message.id) && (
                      <div className="pl-3 py-1 my-2 border-l-2 border-slate-300 text-slate-500 text-sm bg-slate-50/50 rounded-r-lg">
                        <div className="whitespace-pre-wrap break-words leading-relaxed italic font-mono text-xs">
                          {parsedThinking.regularThinking}
                        </div>
                      </div>
                    )}
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-slate-600 ml-1">
                            {message.thinking ? "thinking..." : "chatting..."}
                          </span>
                        </div>
  ...
  (prevProps, nextProps) => {
    ...
    const prevWhiteboardInfo = prevProps.whiteboardInfo.get(
      prevProps.message.id,
    );
    const nextWhiteboardInfo = nextProps.whiteboardInfo.get(
    ...
    return (
      ...
      prevProps.expandedThinking.has(prevProps.message.id) ===
        nextProps.expandedThinking.has(nextProps.message.id) &&
      prevProps.expandedThinking.has(`reactions-${prevProps.message.id}`) ===
        nextProps.expandedThinking.has(`reactions-${nextProps.message.id}`) &&
      prevProps.copiedMessageId === nextProps.copiedMessageId &&
      prevWhiteboardInfo?.renderedContent ===
        nextWhiteboardInfo?.renderedContent
    );
  },
);
