...
const StatsDisplay = lazy(() =>
  import("./StatsDisplay").then((m) => ({ default: m.StatsDisplay })),
);
const WorldItemBrowser = lazy(() =>
  import("./WorldItemBrowser").then((m) => ({ default: m.WorldItemBrowser })),
);
const AgentsList = lazy(() =>
  import("./AgentsList").then((m) => ({ default: m.AgentsList })),
);
const LocationListPanel = lazy(() =>
  import("./LocationListPanel").then((m) => ({ default: m.LocationListPanel })),
);
...
export function GameStatePanel() {
  ...
  const { playerState, world, isChatMode, worldItems } = useGame();
  const [activeTab, setActiveTab] = useState<Tab>("locations");
  const [collapsed, setCollapsed] = useState(() => {
    return localStorage.getItem("gameStatePanelCollapsed") === "true";
  });

  // Get set of item IDs the player owns for WorldItemBrowser
  const playerInventoryIds = useMemo(() => {
    if (!playerState?.inventory) return new Set<string>();
    return new Set(playerState.inventory.map((item) => item.id));
      ...
      >
        {TABS.map((tab) => (
          ...
          >
            ...
            {activeTab === tab.id && (
              <span className="sr-only">{t("accessibility.selectedTab")}</span>
            )}
          </button>
        ))}
      </div>
        ...
        <Suspense fallback={<TabLoadingFallback />}>
          ...
          {activeTab === "inventory" && (
            ...
            >
              <WorldItemBrowser
                items={worldItems}
                playerInventoryIds={playerInventoryIds}
              />
            </div>
          )}
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-bold text-slate-700 tabular-nums">
                ...
                <span className="text-slate-400 ml-1">
                  {t("common.day", { day: playerState.game_time.day })}
                </span>
              </span>
            </div>
          )}
