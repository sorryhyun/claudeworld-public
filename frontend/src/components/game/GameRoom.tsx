...
const formatGameTime = (gameTime: {
  ...
  return `${hours}:${minutes} (Day ${gameTime.day})`;
};
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            {message.game_time_snapshot ? (
              <span className="text-xs text-slate-400">
                {formatGameTime(message.game_time_snapshot)}
              </span>
            ) : (
              message.timestamp && (
                <span className="text-xs text-slate-400">
...
export function GameRoom() {
  ...
  const itemContent = useCallback(
    (_index: number, message: GameMessage) => (
      <GameMessageRow
        message={message}
            ...
            {previousTurnCount > 0 && (
              ...
              >
                ...
                <span
                  ...
                  aria-label={`${previousTurnCount} messages`}
                >
                  {previousTurnCount}
                </span>
              </button>
      ...
      <div aria-live="polite" aria-atomic="true" className="sr-only">
        {displayMessages.length > 0 &&
          displayMessages[displayMessages.length - 1].role !== "user" && (
            <p>
              {t("accessibility.newMessage", {
                agent:
                  displayMessages[displayMessages.length - 1].agent_name ||
                  "Narrator",
              })}
            </p>
          )}
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
              <div>
                ...
                >
                  {t("gameRoom.conversationHistory")}
                </h2>
                <p className="text-sm text-slate-500">
                  {t("gameRoom.messagesTotal", { count: messages.length })}
                </p>
              </div>
              <button
                onClick={() => setShowHistory(false)}
