  ...
  return (
    ...
    >
      <div className="p-3">
        ...
        {/* Travel indicator */}
        {canTravel && (
          <p className="text-xs text-blue-600 mt-1.5 flex items-center gap-1" aria-hidden="true">
...
export function LocationListPanel() {
  ...
  const saveLabel = useCallback(async (locationId: number) => {
    if (editLabel.trim()) {
  ...
  const handleTravel = useCallback(async (location: Location) => {
  ...
  const itemContent = useCallback(
    ...
    [discoveredLocations, currentLocation?.id, adjacentIds, actionInProgress, editingId, editLabel, handleLabelEdit, handleEditLabelChange, saveLabel, cancelEdit, handleTravel, t]
  );

  if (discoveredLocations.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center text-slate-400 text-sm p-4 text-center" role="status">
        <div>
  ...
  const shouldVirtualize = discoveredLocations.length >= VIRTUALIZATION_THRESHOLD;

  if (shouldVirtualize) {
    return (
      <div className="flex-1 h-full" role="list" aria-label={t('gameState.tabs.places')}>
        <Virtuoso
  ...
  return (
    <div className="flex-1 overflow-y-auto" role="list" aria-label={t('gameState.tabs.places')}>
