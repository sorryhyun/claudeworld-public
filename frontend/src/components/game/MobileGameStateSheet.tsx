...
const InventoryList = lazy(() =>
  import("./InventoryList").then((m) => ({ default: m.InventoryList })),
);
...
export function MobileGameStateSheet() {
  ...
  const healthStat = world?.stat_definitions?.stats?.find((s) =>
    ["health", "hp", "vitality", "life"].some((h) =>
      s.name.toLowerCase().includes(h),
    ),
  );
  const healthValue = healthStat
    ? (playerState?.stats?.[healthStat.name] ?? healthStat.default)
    : null;
  const healthMax = healthStat?.max;
  const healthPercent =
    healthValue !== null && healthMax
      ? Math.round((healthValue / healthMax) * 100)
      : null;
      ...
      {isOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          >
            ...
            {/* Tabs */}
            <div
              className="flex border-b border-slate-200 px-2"
              role="tablist"
